...
| A contact's `dob` reads back as **ISO `YYYY-MM-DD`** (#92, 2026-08-20) | No time component; the exact string it was written with. `matchStudent` compares it to `parse.js`'s ISO as a plain string, so this holds — and **no date normalisation belongs in the contact search**. Adding one re-opens the orientation ambiguity a wrong guess would turn into a permanent duplicate per student |
| Contact filters **narrow server-side** (#92, inferred 2026-08-20) | A `?last_name=&dob=` search returned instead of tripping the Worker's page ceiling, which is only possible if at least one filter is honoured — with both ignored, `/prospects` alone would have answered three full pages. Which one narrows, and whether `last_name` is exact or partial, is still open |
